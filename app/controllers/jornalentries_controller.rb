class JornalentriesController < ApplicationController
  ...
  def index
    @jornalentries = Jornalentry.order(date: :desc).order(:tested_at).paginate(page: params[:page], per_page: 10)
  end
  ...
  private
    ...
    def jornalentry_params
      params.require(:jornalentry).permit(:date, :list_of_food, :glucose_mg_dl, :notes, :tested_at)
    end
end
